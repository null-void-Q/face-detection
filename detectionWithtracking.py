...
def main():
    ...
    for vid in vids:
        ...
        if os.path.exists(outputDir+vid.split('/')[-1]):
            continue
        s= cv2.getTickCount()
        extractFaceFromVideo(vid,detector,tracker,outputDir+vid.split('/')[-1])
        print((cv2.getTickCount()-s)/cv2.getTickFrequency())
    #
    print((cv2.getTickCount()-st)/cv2.getTickFrequency())

def extractFaceFromVideo(video,detector,tracker,outvideoPath,dim=(341,256)):
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            if not isFaceDetected:
                detectedFaces = detector.detect_faces(frame)
                if len(detectedFaces) == 0:
                    continue
                isFaceDetected = True
                mainface = getMaxDetection(detectedFaces)
                face = extractFace(frame,mainface['box'])
                (x, y, w, h) = [int(v) for v in mainface['box']]
                tracker.init(frame,(x, y, w, h))
            else:
                (success, box) = tracker.update(frame)
                if success:
                    face = extractFace(frame,[int(v) for v in box])
                else:
                    continue        
            try:
